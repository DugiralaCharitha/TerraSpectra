"""
WEEK 3 DELIVERABLE: Train the Spectral Vision Transformer (ViT)
-----------------------------------------------------------------
1. Loads 16-band PCA cubes (from Week 1).
2. Batches 3D spatial-spectral tensors safely for CPU.
3. Completely immune to corrupt/truncated files via robust fallback.
4. Trains SpectralViT with Multi-Head Self-Attention on Healthy (0) vs Chemically Stressed (1).
5. Saves best checkpoint to models/best_vit.pt.
"""
...
class HyperspectralViTDataset(Dataset):
...
def train_week3_vit(epochs: int = 10, batch_size: int = 16):
    print("=" * 65)
    print("✨ WEEK 3: TRAINING SPECTRAL VISION TRANSFORMER (ViT)")
    print("=" * 65)
    
    if not PCA_PATH.exists():
        raise FileNotFoundError(f"PCA model not found at: {PCA_PATH}. Run Week 1 first.")
    ...
    train_dataset = HyperspectralViTDataset(train_df, pca)
    val_dataset = HyperspectralViTDataset(val_df, pca)
    ...
    print("\nStarting Transformer Training (10 Epochs)...")
    best_val_acc = 0.0
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), MODELS_DIR / "best_vit.pt")
            
    print("=" * 65)
    print(f"🎉 WEEK 3 COMPLETE: Best Spectral ViT Validation Accuracy = {best_val_acc:.2f}%")
    print(f"💾 Checkpoint saved to: {MODELS_DIR / 'best_vit.pt'}")
    print("=" * 65)


if __name__ == "__main__":
    train_week3_vit(epochs=10, batch_size=16)
